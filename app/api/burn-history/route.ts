...
export async function GET(request: NextRequest) {
  try {
    ...
    const estimatedBlocks = Math.min(Math.ceil(periodMs / 12000), limit)
    const startBlockNumber = Math.max(latestBlockNumber - estimatedBlocks, 0)
